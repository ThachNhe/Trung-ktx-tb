...
\begin{document}
	...
	\begin{abstract}
		...
		Trong bối cảnh chuyển đổi số nông nghiệp đang diễn ra mạnh mẽ, nhu cầu giám sát các chỉ số môi trường như nhiệt độ, độ ẩm không khí và độ ẩm đất theo thời gian thực ngày càng trở nên cấp thiết. Đối với các trang trại quy mô nhỏ và vừa, việc theo dõi thủ công thường thiếu tính liên tục, chậm phản ứng khi có bất thường và khó tổng hợp thành báo cáo phục vụ ra quyết định. Vì vậy, nhu cầu xây dựng một hệ thống giám sát nông nghiệp thông minh, trực quan và dễ vận hành là hoàn toàn cần thiết.
		
		Đề tài này tập trung phân tích và mô tả hệ thống \textbf{AgriSmart} - một ứng dụng web hỗ trợ giám sát nông trại với hai vai trò chính là quản trị viên và nông dân. Hệ thống cho phép người dùng đăng nhập an toàn, quản lý thiết bị theo từng trang trại, xem dữ liệu cảm biến theo ngày, theo dõi cảnh báo vượt ngưỡng, đánh dấu cảnh báo đã đọc, xuất nhanh báo cáo CSV và quản lý tài khoản nông dân. Phần backend được xây dựng bằng FastAPI, sử dụng PostgreSQL làm cơ sở dữ liệu; phần frontend được phát triển với React, TypeScript và Vite; toàn bộ hệ thống được đóng gói bằng Docker Compose để thuận lợi cho triển khai.
		
		Một điểm đáng chú ý của hệ thống là cơ chế tác vụ nền sinh dữ liệu cảm biến giả lập định kỳ, phục vụ việc kiểm thử giao diện dashboard và luồng cảnh báo ngay cả khi chưa kết nối thiết bị IoT thật. Nhờ đó, AgriSmart có thể được dùng như một nền tảng mẫu để tiếp tục mở rộng sang các tình huống thực tế như kết nối cảm biến ngoài đồng ruộng, cảnh báo đa kênh hoặc điều khiển thiết bị chấp hành.
		
		\textbf{Nội dung, phạm vi của đề tài}
		\begin{itemize}
			\item \textbf{Nội dung:} Khảo sát nhu cầu giám sát nông nghiệp, phân tích yêu cầu, thiết kế dữ liệu và mô tả việc xây dựng một hệ thống web theo dõi thiết bị, dữ liệu cảm biến và cảnh báo cho mô hình nông trại thông minh.
			\item \textbf{Phạm vi của đề tài:} Tập trung vào phiên bản web phục vụ quản trị viên và nông dân; dữ liệu cảm biến gồm nhiệt độ, độ ẩm không khí và độ ẩm đất; cảnh báo được tạo khi vượt ngưỡng đã định; hệ thống hỗ trợ xuất báo cáo CSV, quản lý farmer và phân quyền cơ bản. Phiên bản hiện tại chưa triển khai điều khiển thiết bị vật lý và chưa kết nối trực tiếp đến cảm biến ngoài thực địa.
		\end{itemize}
	\end{abstract}
...
%	\addcontentsline{toc}{section}{DANH MỤC TỪ VIẾT TẮT}
	...
	\begin{table}[H]
		...
		\begin{tabular}{|c|p{11cm}|}
			...
			\hline
			AgriSmart & Hệ thống giám sát nông nghiệp thông minh của đề tài. \\
			\hline
			...
			\hline
			CSV & Định dạng tệp dùng để xuất báo cáo dữ liệu cảm biến. \\
			\hline
			CRUD & Create, Read, Update, Delete, bốn thao tác cơ bản với dữ liệu. \\
			\hline
			DB & Cơ sở dữ liệu PostgreSQL lưu trữ người dùng, thiết bị, cảnh báo và số liệu cảm biến. \\
			\hline
			FE & Frontend, giao diện người dùng được xây dựng bằng React, TypeScript và Vite. \\
			\hline
			HttpOnly & Thuộc tính cookie chỉ có thể được truy cập qua HTTP, dùng để bảo vệ token đăng nhập. \\
			\hline
			IoT & Internet of Things, mô hình kết nối cảm biến và thiết bị trong nông trại. \\
			\hline
			JWT & JSON Web Token, chuẩn token dùng cho xác thực và phân quyền. \\
			\hline
			ORM & Object Relational Mapping, cơ chế ánh xạ đối tượng với bảng trong CSDL. \\
			\hline
			UUID & Mã định danh duy nhất toàn cục, dùng cho user, device, alert và sensor reading. \\
			\hline
		\end{tabular}
	\end{table}
...
\addcontentsline{toc}{section}{1.1 Tổng quan về hệ thống đề xuất}


\subsubsection*{1.1.1. Nghiên cứu tổng quan về hệ thống quản lý ký túc xá trên thế giới}
\addcontentsline{toc}{section}{1.1.1 Nghiên cứu tổng quan về hệ thống quản lý ký túc xá trên thế giới}
...
\subsubsection*{1.1.2. Nghiên cứu tổng quan về hệ thống quản lý ký túc xá Việt Nam}
\addcontentsline{toc}{section}{1.1.2. Nghiên cứu tổng quan về hệ thống quản lý ký túc xá Việt Nam}


Tại Việt Nam, nhu cầu số hóa công tác quản lý ký túc xá đã xuất hiện ở nhiều trường đại học, đặc biệt tại các đơn vị có số lượng sinh viên nội trú lớn. Tuy nhiên, mức độ hoàn thiện của các hệ thống hiện có còn khá chênh lệch. Ở một số cơ sở giáo dục lớn, nghiệp vụ nội trú đã được tích hợp vào cổng thông tin sinh viên hoặc hệ thống quản trị đào tạo. Trong các trường hợp này, sinh viên có thể tra cứu danh sách phòng, theo dõi nghĩa vụ tài chính hoặc nhận các thông báo chung qua môi trường trực tuyến. Dù vậy, phạm vi chức năng thường tập trung vào các nghiệp vụ hành chính cơ bản, trong khi các bài toán như xử lý bảo trì, quản lý trạng thái phòng theo thời gian thực hoặc thống kê vận hành chi tiết chưa phải lúc nào cũng được hỗ trợ đầy đủ.

Ở nhiều đơn vị khác, đặc biệt tại những trường có điều kiện hạ tầng công nghệ chưa đồng đều, công tác quản lý nội trú vẫn còn phụ thuộc mạnh vào bảng tính điện tử, hồ sơ giấy, tin nhắn hoặc trao đổi trực tiếp giữa sinh viên với bộ phận quản lý. Cách làm này có ưu điểm là dễ triển khai trong giai đoạn đầu, nhưng càng về lâu dài càng bộc lộ hạn chế về tính chính xác, khả năng truy vết và tốc độ tổng hợp báo cáo. Khi số lượng sinh viên tăng lên hoặc cơ cấu khu nhà trở nên phức tạp hơn, việc quản lý thủ công rất dễ dẫn tới nhầm lẫn trong phân phòng, bỏ sót hóa đơn, khó đối soát tình trạng cư trú và chậm trễ trong phản hồi các yêu cầu phát sinh.

Đối với các trường ở khu vực miền núi hoặc có số lượng lớn sinh viên ngoại tỉnh và lưu học sinh, bài toán ký túc xá còn có thêm đặc thù về đối tượng cư trú, nhu cầu phân loại phòng ở và yêu cầu theo dõi sát sao tình trạng sử dụng phòng. Do đó, một hệ thống quản lý ký túc xá phù hợp với môi trường đại học Việt Nam cần vừa bám sát điều kiện thực tế vận hành, vừa đủ linh hoạt để phản ánh các quy tắc quản lý đặc thù của từng đơn vị.

\subsubsection*{1.1.3. Đánh giá chất lượng sản phẩm đã biết}
\addcontentsline{toc}{section}{1.1.3. Đánh giá chất lượng sản phẩm đã biết}
...
\addcontentsline{toc}{section}{1.2. Mục đích nghiên cứu}
...
\addcontentsline{toc}{section}{1.2.1. Tăng cường hiệu quả quản lý và vận hành ký túc xá}
...
\addcontentsline{toc}{section}{1.2.2. Cải thiện trải nghiệm người dùng}
...
\addcontentsline{toc}{section}{1.2.3. Hỗ trợ ra quyết định dựa trên dữ liệu}
...
\addcontentsline{toc}{section}{1.2.4. Tích hợp các công nghệ hiện đại}
...
\addcontentsline{toc}{section}{1.2.5. Giảm chi phí vận hành và quản lý}
...
\addcontentsline{toc}{section}{1.2.6. Đáp ứng nhu cầu của môi trường đại học Việt Nam}

Mục đích cuối cùng của đề tài là xây dựng một hệ thống phù hợp với bối cảnh thực tế của ký túc xá trong trường đại học Việt Nam. Điều đó thể hiện ở việc thiết kế giao diện tiếng Việt, lựa chọn các quy tắc nghiệp vụ gần với hoạt động nội trú thực tế, và tổ chức các chức năng theo vai trò quen thuộc như quản trị viên, nhân viên và sinh viên.
...
\addcontentsline{toc}{section}{1.3. Đối tượng và phạm vi nghiên cứu}

\subsubsection*{1.3.1. Đối tượng nghiên cứu}
\addcontentsline{toc}{section}{1.3.1. Đối tượng nghiên cứu}
...
\addcontentsline{toc}{section}{1.3.2. Phạm vi nghiên cứu}
...
\addcontentsline{toc}{section}{1.4. Phương pháp nghiên cứu}
...
\addcontentsline{toc}{section}{1.4.2. Phương pháp phân tích hệ thống}
...
\addcontentsline{toc}{section}{1.4.3. Phương pháp thiết kế hệ thống}
...
\subsubsection*{1.4.4. Phương pháp lập trình và thử nghiệm}
\addcontentsline{toc}{section}{1.4.4. Phương pháp lập trình và thử nghiệm}
...
Trong môi trường đại học, ký túc xá là bộ phận gắn liền với đời sống học tập và sinh hoạt của một bộ phận lớn sinh viên, đặc biệt là sinh viên ngoại tỉnh, sinh viên có hoàn cảnh khó khăn hoặc lưu học sinh quốc tế. Công tác quản lý ký túc xá vì thế không chỉ dừng lại ở việc bố trí chỗ ở mà còn bao gồm nhiều hoạt động liên quan như theo dõi thông tin cư trú, quản lý sức chứa phòng, thu phí nội trú, xử lý sự cố cơ sở vật chất, bảo đảm thông tin liên lạc và tổng hợp báo cáo cho lãnh đạo nhà trường.

Thực tế cho thấy những công việc này rất dễ bị phân tán nếu không có một hệ thống quản lý thống nhất. Khi dữ liệu phòng ở nằm ở một nơi, hóa đơn ở một nơi khác và phản ánh bảo trì lại được tiếp nhận qua kênh riêng, bộ phận quản lý sẽ khó kiểm soát được tình trạng vận hành chung. Trong khi đó, sinh viên cũng gặp khó khăn trong việc theo dõi thông tin về đơn đăng ký, khoản phải nộp hoặc tình trạng xử lý các yêu cầu đã gửi. Chính vì vậy, việc xây dựng một hệ thống quản lý ký túc xá có tính tập trung, nhất quán và minh bạch là nhu cầu rất cần thiết.

Đề tài được lựa chọn trên cơ sở nhận thấy bài toán nội trú sinh viên có giá trị thực tiễn cao, đủ rõ về phạm vi và đủ phong phú về nghiệp vụ để triển khai thành một hệ thống thông tin hoàn chỉnh. Bên cạnh ý nghĩa ứng dụng, đề tài còn cho phép vận dụng kiến thức về phân tích yêu cầu, thiết kế cơ sở dữ liệu, xây dựng API, phát triển giao diện web và triển khai hệ thống trong môi trường container.

\subsubsection*{1.1.1. Mục tiêu của đề tài}
\addcontentsline{toc}{section}{1.1.1. Mục tiêu của đề tài}
...
\addcontentsline{toc}{section}{1.1.2. Đối tượng và phạm vi nghiên cứu}
...
\addcontentsline{toc}{section}{1.2. Khảo sát thực tế}

\subsubsection*{1.2.1. Kết quả khảo sát}
\addcontentsline{toc}{section}{1.2.1. Kết quả khảo sát}

Kết quả khảo sát nghiệp vụ cho thấy hoạt động quản lý ký túc xá xoay quanh một chuỗi quy trình lặp lại theo chu kỳ. Đầu tiên là quản lý hạ tầng lưu trú, trong đó bộ phận phụ trách cần biết khu nhà nào đang hoạt động, mỗi khu có bao nhiêu phòng, mỗi phòng có sức chứa bao nhiêu, thuộc nhóm đối tượng nào và còn bao nhiêu chỗ trống. Tiếp theo là quy trình tiếp nhận sinh viên nội trú, nơi phát sinh nhu cầu xem phòng phù hợp, nộp đơn đăng ký, xét duyệt, bố trí vào ở và cập nhật tình trạng cư trú.

Trong quá trình sinh viên sinh hoạt tại ký túc xá, bộ phận quản lý còn phải xử lý các nghiệp vụ phát sinh như lập hóa đơn theo tháng, theo dõi tình trạng thanh toán, tiếp nhận phản ánh về hỏng hóc hoặc sự cố phòng ở, đồng thời gửi thông báo chung đến đúng nhóm đối tượng. Ngoài ra, lãnh đạo và ban quản lý cũng có nhu cầu xem báo cáo tổng hợp để đánh giá doanh thu nội trú, công suất sử dụng phòng và mức độ vận hành của từng khu nhà.

Khảo sát trực tiếp trên hệ thống được phát triển trong repo cho thấy sản phẩm đã phản ánh tương đối rõ các nghiệp vụ này. Hệ thống hiện có ba vai trò chính là quản trị viên, nhân viên và sinh viên; có các thực thể dữ liệu cho người dùng, tòa nhà, phòng ở, đăng ký nội trú, hóa đơn, bảo trì và thông báo; đồng thời đã hiện thực một số quy tắc nghiệp vụ đáng chú ý như phân loại phòng theo giới tính hoặc quốc tịch, chống trùng đơn đăng ký, chống trùng hóa đơn và giới hạn phạm vi yêu cầu bảo trì.

\subsubsection*{1.2.2. Phân tích kết quả khảo sát}
\addcontentsline{toc}{section}{1.2.2. Phân tích kết quả khảo sát}
...
\addcontentsline{toc}{section}{1.3. Đặc tả hệ thống}
...
\addcontentsline{toc}{section}{2.1. Các yêu cầu chức năng}

\subsubsection*{2.1.1. Quản lý phòng}
\addcontentsline{toc}{section}{2.1.1. Quản lý phòng}
...
\addcontentsline{toc}{section}{2.1.2. Đặt phòng và trả phòng}
...
\addcontentsline{toc}{section}{2.1.3. Quản lý sinh viên}
...
\addcontentsline{toc}{section}{2.1.4. Quản lý thanh toán}
...
\addcontentsline{toc}{section}{2.1.5. Quản lý người dùng và phân quyền}
...
\addcontentsline{toc}{section}{2.1.6. Báo cáo và thống kê}
...
\addcontentsline{toc}{section}{2.1.7. Thông báo và xác nhận}
...
\addcontentsline{toc}{section}{2.2. Biểu đồ Usecase}
...
Ngoài biểu đồ tổng quát, các use case thành phần cho từng nghiệp vụ cũng được biểu diễn riêng trong file \texttt{UML.txt}. Cách tổ chức này giúp phần báo cáo giữ được tính súc tích trong khi phần sơ đồ vẫn đủ chi tiết để người xem theo dõi mối liên hệ giữa tác nhân và chức năng.

\subsection*{2.3. Đặc tả ca sử dụng}
\addcontentsline{toc}{section}{2.3. Đặc tả ca sử dụng}

\subsubsection*{2.3.1. Ca sử dụng 1: Đăng nhập hệ thống}
\addcontentsline{toc}{section}{2.3.1. Ca sử dụng 1: Đăng nhập hệ thống}
...
\subsubsection*{2.3.2. Ca sử dụng 2: Quản lý phòng}
\addcontentsline{toc}{section}{2.3.2. Ca sử dụng 2: Quản lý phòng}
...
\subsubsection*{2.3.3. Ca sử dụng 3: Đặt phòng}
\addcontentsline{toc}{section}{2.3.3. Ca sử dụng 3: Đặt phòng}
...
\subsubsection*{2.3.4. Ca sử dụng 4: Trả phòng}
\addcontentsline{toc}{section}{2.3.4. Ca sử dụng 4: Trả phòng}
...
\subsubsection*{2.3.5. Ca sử dụng 5: Quản lý dịch vụ}
\addcontentsline{toc}{section}{2.3.5. Ca sử dụng 5: Quản lý dịch vụ}
...
\subsubsection*{2.3.6. Ca sử dụng 6: Quản lý sinh viên}
\addcontentsline{toc}{section}{2.3.6. Ca sử dụng 6: Quản lý sinh viên}
...
\subsubsection*{2.3.7. Ca sử dụng 7: Xem báo cáo -- thống kê}
\addcontentsline{toc}{section}{2.3.7. Ca sử dụng 7: Xem báo cáo -- thống kê}
...
\subsection*{2.4. Phân tích luồng hoạt động của các ca sử dụng}
\addcontentsline{toc}{section}{2.4. Phân tích luồng hoạt động của các ca sử dụng}

\subsubsection*{2.4.1. Xác định các luồng nghiệp vụ trọng tâm}
\addcontentsline{toc}{section}{2.4.1. Xác định các luồng nghiệp vụ trọng tâm}
...
\subsubsection*{2.4.2. Biểu đồ hoạt động cho các ca sử dụng}
\addcontentsline{toc}{section}{2.4.2. Biểu đồ hoạt động cho các ca sử dụng}

\subsubsection*{2.4.2.1. Ca sử dụng: Đặt phòng}
\addcontentsline{toc}{section}{2.4.2.1. Ca sử dụng: Đặt phòng}
...
\subsubsection*{2.4.2.2. Ca sử dụng: Quản lý sinh viên}
\addcontentsline{toc}{section}{2.4.2.2. Ca sử dụng: Quản lý sinh viên}
...
\subsubsection*{2.4.2.3. Ca sử dụng: Quản lý phòng}
\addcontentsline{toc}{section}{2.4.2.3. Ca sử dụng: Quản lý phòng}
...
\subsubsection*{2.4.2.4. Ca sử dụng: Thanh toán}
\addcontentsline{toc}{section}{2.4.2.4. Ca sử dụng: Thanh toán}
...
\subsubsection*{2.4.2.5. Ca sử dụng: Đăng nhập hệ thống}
\addcontentsline{toc}{section}{2.4.2.5. Ca sử dụng: Đăng nhập hệ thống}
...
\subsubsection*{2.4.2.6. Ca sử dụng: Xem báo cáo thống kê}
\addcontentsline{toc}{section}{2.4.2.6. Ca sử dụng: Xem báo cáo thống kê}
...
\subsubsection*{2.4.3. Mô tả luồng chuyển trạng thái của các ca sử dụng}
\addcontentsline{toc}{section}{2.4.3. Mô tả luồng chuyển trạng thái của các ca sử dụng}

\subsubsection*{2.4.3.1. Luồng trạng thái của đơn đăng ký nội trú}
\addcontentsline{toc}{section}{2.4.3.1. Luồng trạng thái của đơn đăng ký nội trú}

Đơn đăng ký nội trú trong hệ thống chuyển động theo bốn trạng thái chính là chờ xử lý, đã duyệt, bị từ chối và đã trả phòng. Trạng thái chờ xử lý là trạng thái khởi tạo khi sinh viên nộp đơn hợp lệ. Từ đây, nhân viên có thể đưa đơn sang trạng thái đã duyệt hoặc bị từ chối. Nếu đơn đã được duyệt và sinh viên kết thúc thời gian cư trú, trạng thái sẽ chuyển sang đã trả phòng. Việc mô tả chuỗi trạng thái này giúp bảo đảm rằng cùng một bản ghi không thể được xử lý theo các hướng mâu thuẫn nhau.

\subsubsection*{2.4.3.2. Luồng trạng thái của yêu cầu bảo trì}
\addcontentsline{toc}{section}{2.4.3.2. Luồng trạng thái của yêu cầu bảo trì}

Yêu cầu bảo trì được khởi tạo ở trạng thái chờ xử lý, sau đó có thể chuyển sang đang xử lý và cuối cùng là đã hoàn thành. Chuỗi trạng thái này phản ánh đúng nhịp vận hành thực tế của công việc kỹ thuật trong ký túc xá. Mặc dù báo cáo không xây dựng biểu đồ trạng thái riêng, luồng này vẫn được mô tả bằng lời để làm rõ logic chuyển đổi bên trong hệ thống.

\subsubsection*{2.4.3.3. Luồng trạng thái của phòng ở}
\addcontentsline{toc}{section}{2.4.3.3. Luồng trạng thái của phòng ở}

Phòng ở trong hệ thống có ba trạng thái vận hành chính là còn chỗ, đã đầy và đang bảo trì. Trạng thái của phòng thay đổi theo số lượng sinh viên hiện diện và tình trạng kỹ thuật của phòng. Khi số người ở đạt mức tối đa, trạng thái chuyển sang đã đầy. Khi phòng phát sinh vấn đề kỹ thuật cần ngừng tiếp nhận người mới, trạng thái chuyển sang đang bảo trì. Luồng trạng thái này tác động trực tiếp tới khả năng cho phép đăng ký mới.

\subsubsection*{2.4.3.4. Luồng trạng thái của hóa đơn}
\addcontentsline{toc}{section}{2.4.3.4. Luồng trạng thái của hóa đơn}

Hóa đơn có hai trạng thái cơ bản là chưa thanh toán và đã thanh toán. Đây là mô hình trạng thái đơn giản nhưng quan trọng vì nó ảnh hưởng trực tiếp tới báo cáo doanh thu. Khi hóa đơn đã được xác nhận thanh toán, hệ thống lưu thời điểm thanh toán để phục vụ đối soát về sau.

\subsubsection*{2.4.3.5. Luồng trạng thái của phiên đăng nhập}
\addcontentsline{toc}{section}{2.4.3.5. Luồng trạng thái của phiên đăng nhập}

Phiên đăng nhập có thể được hiểu qua hai trạng thái chính là còn hiệu lực và hết hiệu lực. Khi người dùng đăng nhập thành công, hệ thống sinh token xác thực và coi phiên là hợp lệ. Khi access token hết hạn, hệ thống có thể làm mới thông qua refresh token. Nếu người dùng đăng xuất hoặc refresh token không còn hợp lệ, phiên làm việc kết thúc. Luồng này đóng vai trò quan trọng trong bảo mật truy cập.

\subsubsection*{2.4.3.6. Luồng trạng thái của báo cáo theo tháng}
\addcontentsline{toc}{section}{2.4.3.6. Luồng trạng thái của báo cáo theo tháng}

Khác với các thực thể trên, báo cáo không có trạng thái lưu trữ dài hạn mà được tạo động theo yêu cầu xem dữ liệu. Tuy nhiên, về mặt xử lý có thể hình dung luồng của báo cáo qua ba bước là tiếp nhận tham số thời gian, tổng hợp dữ liệu nguồn và trả về kết quả hiển thị. Cách mô tả này giúp làm rõ đặc điểm của báo cáo như một sản phẩm tổng hợp được sinh ra từ dữ liệu vận hành hiện có.

\subsection*{2.5. Biểu đồ tuần tự cho các ca sử dụng}
\addcontentsline{toc}{section}{2.5. Biểu đồ tuần tự cho các ca sử dụng}

\subsubsection*{2.5.1. Biểu đồ tuần tự đăng nhập hệ thống}
\addcontentsline{toc}{section}{2.5.1. Biểu đồ tuần tự đăng nhập hệ thống}
...
\addcontentsline{toc}{section}{2.5.2. Biểu đồ tuần tự quản lý phòng}
...
\addcontentsline{toc}{section}{2.5.3. Biểu đồ tuần tự đặt phòng}
...
\addcontentsline{toc}{section}{2.5.4. Biểu đồ tuần tự thanh toán}
...
\addcontentsline{toc}{section}{2.5.5. Biểu đồ tuần tự quản lý bảo trì và thông báo}
...
\addcontentsline{toc}{section}{2.5.6. Biểu đồ tuần tự xem báo cáo thống kê}
...
\subsection*{3.1. Biểu đồ triển khai}
\addcontentsline{toc}{section}{3.1. Biểu đồ triển khai}

Biểu đồ triển khai của hệ thống được xây dựng theo mô hình ba thành phần chính gồm frontend, backend và cơ sở dữ liệu PostgreSQL. Frontend là ứng dụng web được người dùng truy cập qua trình duyệt. Backend đóng vai trò máy chủ ứng dụng, cung cấp các API phục vụ mọi nghiệp vụ. Cơ sở dữ liệu là nơi lưu trữ toàn bộ dữ liệu người dùng, phòng ở, đăng ký, hóa đơn, bảo trì và thông báo. Trong môi trường phát triển, ba thành phần này được tổ chức và khởi chạy bằng Docker Compose để bảo đảm tính nhất quán cấu hình.

Biểu đồ triển khai có ý nghĩa quan trọng vì nó cho thấy cách hệ thống được tổ chức ở mức hạ tầng. Việc tách biệt frontend, backend và database không chỉ giúp dễ bảo trì mà còn tạo điều kiện thuận lợi cho việc mở rộng, nâng cấp hoặc triển khai trên các môi trường khác nhau trong tương lai.

\subsection*{3.2. Kiến trúc hệ thống}
\addcontentsline{toc}{section}{3.2. Kiến trúc hệ thống}

\subsubsection*{3.2.1. Lớp trình diễn}
\addcontentsline{toc}{section}{3.2.1. Lớp trình diễn}

Lớp trình diễn của hệ thống được xây dựng bằng React kết hợp Vite và TypeScript. Đây là lớp trực tiếp tương tác với người dùng, có nhiệm vụ hiển thị dữ liệu, tiếp nhận thao tác, kiểm tra hợp lệ của biểu mẫu ở mức giao diện và gửi yêu cầu đến backend. Giao diện được tổ chức theo vai trò, giúp sinh viên, nhân viên và quản trị viên nhìn thấy đúng khu vực chức năng thuộc phạm vi sử dụng của mình.

Ngoài vai trò hiển thị, lớp trình diễn còn hỗ trợ điều hướng theo quyền truy cập thông qua cơ chế route guard. Nhờ vậy, ngay cả khi người dùng cố truy cập trực tiếp bằng đường dẫn, hệ thống vẫn có thể điều hướng về đúng khu vực hoặc từ chối truy cập nếu vai trò không phù hợp.

\subsubsection*{3.2.2. Lớp xử lí nghiệp vụ}
\addcontentsline{toc}{section}{3.2.2. Lớp xử lí nghiệp vụ}

Lớp xử lí nghiệp vụ được hiện thực trong backend FastAPI và là trung tâm logic của toàn hệ thống. Tại đây, các dịch vụ chịu trách nhiệm kiểm tra điều kiện nghiệp vụ, xử lý dữ liệu đầu vào, cập nhật trạng thái và trả kết quả cho giao diện. Các service như \texttt{AuthService}, \texttt{BuildingRoomService}, \texttt{RegistrationService}, \texttt{InvoiceService}, \texttt{MaintenanceService}, \texttt{NotificationService} và \texttt{ReportService} được tổ chức theo từng miền chức năng rõ ràng.

Việc tập trung nghiệp vụ tại lớp này giúp hệ thống tránh tình trạng logic bị rải rác ở nhiều nơi. Đây là lựa chọn kiến trúc hợp lý vì cùng một quy tắc có thể được tái sử dụng cho nhiều giao diện hoặc nhiều tình huống truy cập khác nhau mà vẫn bảo đảm tính nhất quán.

\subsubsection*{3.2.3. Lớp dữ liệu}
\addcontentsline{toc}{section}{3.2.3. Lớp dữ liệu}

Lớp dữ liệu sử dụng PostgreSQL làm hệ quản trị cơ sở dữ liệu quan hệ, kết hợp với SQLAlchemy để ánh xạ giữa bảng dữ liệu và đối tượng nghiệp vụ trong mã nguồn. Các migration được quản lý bằng Alembic nhằm theo dõi lịch sử thay đổi cấu trúc cơ sở dữ liệu một cách có kiểm soát. Nhờ đó, hệ thống có thể phát triển dần mà không làm mất tính nhất quán giữa mã nguồn và mô hình dữ liệu.

Lớp dữ liệu không chỉ lưu trữ thông tin mà còn là nơi bảo đảm tính toàn vẹn thông qua khóa chính, khóa ngoại, ràng buộc duy nhất và các trường trạng thái. Khi kết hợp với lớp xử lý nghiệp vụ, hệ thống vừa có thể ngăn lỗi ở tầng ứng dụng, vừa duy trì được cấu trúc dữ liệu chặt chẽ ở tầng lưu trữ.

\subsection*{3.3. Cơ sở dữ liệu}
\addcontentsline{toc}{section}{3.3. Cơ sở dữ liệu}
...
\addcontentsline{toc}{section}{3.4. Giao diện của hệ thống}

\subsubsection*{3.4.1. Giao diện đăng nhập}
\addcontentsline{toc}{section}{3.4.1. Giao diện đăng nhập}
...
\addcontentsline{toc}{section}{3.4.3. Giao diện quản lý dịch vụ}
...
\addcontentsline{toc}{section}{3.4.4. Giao diện quản lý phòng}
...
\addcontentsline{toc}{section}{3.4.6. Giao diện báo cáo thống kê}
